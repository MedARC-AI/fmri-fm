...
class HCPParcPTDataset(Dataset):
    ...
    def __init__(
        self,
        pt_dir: str,
        num_frames: int = 160, # Changed to 160 for pretraining
        ...
        # Find all .pt files and drop those with less than minimum frames
        all_pt_files = sorted(self.pt_dir.glob("*.pt"))
        if not all_pt_files:
            raise RuntimeError(f"No .pt files found in {pt_dir}")
        
        # Filter files by minimum frame count (160)
        self.pt_files: List[Path] = []
        filtered_count = 0
        
        logger.info(f"Filtering files with minimum 160 frames...")
        for pt_file in all_pt_files:
            try:
                # Fast filtering: only load metadata, not the full tensor
                data = torch.load(pt_file, map_location='cpu', weights_only=True)
                
                # Check timeframes from fmri shape (faster than loading full data)
                if 'fmri' in data:
                    ...
                    logger.warning(f"No 'fmri' key in {pt_file.name}")
                    filtered_count += 1
            except Exception as e:
                logger.warning(f"Error checking {pt_file.name}: {e}")
                filtered_count += 1
        
        if not self.pt_files:
            raise RuntimeError(f"No .pt files found with >= 160 frames in {pt_dir}")
        
        logger.info(f"HCPParcPTDataset: found {len(self.pt_files)} samples (filtered out {filtered_count} short samples) in {pt_dir}")
    ...
    def __getitem__(self, idx: int):
        ...
        if self.downsample:
            ...
            # Simple random clip sampling for pretraining
            # Random start position for clip sampling
            start_idx = random.randint(0, T - self.num_frames)
            end_idx = start_idx + self.num_frames 
            ts = ts[:, start_idx:end_idx]
        
        # Optional per-sample standardization (usually not used with Brain-JEPA's dataset -level robust scaling)
...
if __name__ == "__main__":
    ...
    pt_dir = sys.argv[1]
    dataset = HCPParcPTDataset(pt_dir=pt_dir, num_frames=160, downsample=False)
