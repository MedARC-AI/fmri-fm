...
from brain_jepa.masks.utils import apply_masks
from brain_jepa.utils.distributed import (
    init_distributed,
    AllReduce
)
from brain_jepa.utils.logging import (
    CSVLogger,
    gpu_timer,
    grad_logger,
    AverageMeter)
from brain_jepa.utils.tensors import repeat_interleave_batch
from brain_jepa.datasets.hcp_parc import make_hcp  # HCP dataloader returning {'fmri': tensor}
from brain_jepa.datasets.hcp_parc_pt import make_hcp_pt  # HCP PT dataloader

from brain_jepa.helper import (
    load_checkpoint,
    init_model,
    init_opt)
...
def main(args, resume_preempt=False):
    ...
    tar_glob = args['data'].get('tar_glob', '/teamspace/filestore_folders/shared/fmri-fm/datasets/hcp-parc/hcp-parc_*.tar')
    
    # Check dataset type
    dataset_type = args['meta'].get('dataset_type', 'hcp_tar')
    pt_dir = args['meta'].get('pt_dir', None)
    
    # Wandb configuration
    use_wandb = args['meta'].get('use_wandb', False)
    wandb_project = args['meta'].get('wandb_project', 'brain-jepa-hcp')
    ...
    if use_wandb and rank == 0:
        run_name = wandb_run_name or f"{tag}_{datetime.now().strftime('%y%m%d-%H%M%S')}"
        wandb.init(
    ...
    if mask_mode == 'roi_mask':
        from brain_jepa.masks.spatialtemporal_multiblock import MaskCollator_fmri as MBMaskCollator
    ...
    if dataset_type == 'hcp_pt':
        if pt_dir is None:
            raise ValueError("pt_dir must be specified when using dataset_type='hcp_pt'")
        _, unsupervised_loader, unsupervised_sampler = make_hcp_pt(
                pt_dir=pt_dir,
                ...
                use_standardization=use_standatdization)
    else:  # Default to tar-based loader
        _, unsupervised_loader, unsupervised_sampler = make_hcp(
                tar_glob=tar_glob,
                batch_size=batch_size,
                collator=mask_collator,
                pin_mem=pin_mem,
        ...
        for itr, (udata, masks_enc, masks_pred) in enumerate(unsupervised_loader):
            ...
            def train_step():
                ...
                if torch.cuda.is_available():
                    ...
                    with torch.amp.autocast('cpu', dtype=torch.bfloat16, enabled=use_bfloat16):
                        h = forward_target()
                        z = forward_context()
                        loss = loss_fn(z, h)
            ...
            def log_stats():
                ...
                if use_wandb and rank == 0:
                    wandb_log_dict = {
                        ...
                        'maskA_avg': maskA_meter.avg,
                        'maskB': maskB_meter.val,
                        'maskB_avg': maskB_meter.avg,
                        'learning_rate': _new_lr,
                        'weight_decay': _new_wd,
                        'time_ms': etime,
                        'time_avg_ms': time_meter.avg,
                    }
                    if torch.cuda.is_available():
                        wandb_log_dict['gpu_memory_mb'] = torch.cuda.max_memory_allocated() / 1024.**2
                    ...
                    wandb.log(wandb_log_dict, step=itr + epoch * ipe)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg-path", type=str, default="src/brain_jepa/config/debug_train.yaml")
