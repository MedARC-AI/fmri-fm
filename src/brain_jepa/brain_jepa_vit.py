...
class GradTs_2dPE(nn.Module):
    def __init__(
        self, in_chan, embed_dim, grid_size, add_w=False, cls_token=False
    ) -> None:
        super().__init__()
        ...
        pos_emb_h = self.get_1d_sincos_pos_embed_from_grid(
        ...
        if add_w == "origin":
            ...
            pos_emb_w = self.get_1d_sincos_pos_embed_from_grid(
    ...
    def forward(
        self, gradient
    ):  # TODO: check which one is emb_w and which one is emb_h !!!!!
        ...
        if self.cls_token:
            pos_embed = torch.concat(
                [torch.zeros([1, 1, emb.shape[2]]).cuda(), emb], dim=1
            )
        else:
            pos_embed = emb
        return pos_embed
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        norm = self.norm1(x)  # .to(torch.float16)
        y, attn = self.attn(norm, return_attention)
...
class VisionTransformerPredictor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.predictor_2dpe_proj = GradTs_2dPE(
            gradient_pos_embed.shape[-1],
            predictor_embed_dim,
            num_patches_2d,
            add_w=add_w,
            cls_token=False,
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gradient_pos_embed = gradient_pos_embed
        self.pos_embed_proj = GradTs_2dPE(
            gradient_pos_embed.shape[-1],
            embed_dim,
            self.num_patches_2d,
            add_w=add_w,
    ...
    def forward(self, x, masks=None, return_attention=False):
        ...
        print(self.patch_embed.proj.bias.dtype)  # float32
        # print(self.patch_embed.num_patches)#4500?
        # print(self.patch_embed.patch_size)#16
        x = self.patch_embed(x)

        B, N, D = x.shape
        # N * d = 4500. Not sure what they should actually be though.
        print(x.shape)
        ...
        for i, blk in enumerate(self.blocks):
            if self.gradient_checkpointing and self.training:
                ...
                if return_attention:
                    ...
                    # x = x.to(torch.float16)#Not sure why this is needed tbh
                    x = blk(x)
