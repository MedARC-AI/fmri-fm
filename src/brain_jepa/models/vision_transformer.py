...
from brain_jepa.utils.tensors import (
    trunc_normal_,
...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attn=None):
        ...
        if self.attn_mode == 'normal':
            ...
            # the flash_attn package is too brittle to install
            ...
            # nb, copying the authors typo to use proj_drop_rate instead of attn_drop.p,
            # and ignore whether this is training or not.
            x = F.scaled_dot_product_attention(
                q, k, v,
                dropout_p=self.proj_drop_rate,
            )
            attn = None
        else:
            raise NotImplementedError(f"{self.attn_mode} not supported")
        x = x.reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        if return_attn:
