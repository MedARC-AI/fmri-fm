...
from flat_mae.masking import mask_collate

from flat_capi.adapters import CapiBackboneAdapter, create_adapter_from_ckpt
...
def main(args: DictConfig):
    ...
    print("probe eval flat map capi")
    ...
    if args.pretrain_ckpt:
        print(f"creating CAPI backbone adapter from checkpoint: {args.pretrain_ckpt}")
        adapter, ckpt_args = create_adapter_from_ckpt(
            args.pretrain_ckpt,
            device=device,
            prefer_ema=True,
            time_as_channels=getattr(args, "time_as_channels", None),
            select_frame_index=getattr(args, "select_frame_index", None),
        )
        backbone = adapter
    else:
        print(f"creating CAPI backbone from scratch: {args.model}")
        model_ctor = getattr(models_capi, args.model, models_capi.vit_l14_capi)
        raw_backbone = model_ctor(**args)
        backbone = CapiBackboneAdapter(
            raw_backbone,
    ...
    log_classifier_keys = [
        (feature_source, (mid_lr_scale, mid_weight_decay)) for feature_source in args.representations
    ]
    ...
    best_classifiers = {
        (feature_source, hparam): model_without_ddp.classifiers[idx]
        for idx, (feature_source, hparam) in enumerate(model_without_ddp.classifier_keys)
        if best_hparams.get(feature_source) == hparam
    }
    ...
    if test_loader is not None:
        ...
        if args.distributed:
            best_model = torch.nn.parallel.DistributedDataParallel(best_model, device_ids=[args.gpu])
        test_stats = evaluate(
            args,
            best_model,
            criterion,
